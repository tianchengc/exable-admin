import { Button, Form, message, Modal, Popconfirm, Tag, Tree, Upload, UploadProps } from 'antd'
...
export const KinList: FunctionComponent<{}> = () => {
    ...
    const columns: ColumnsType<IAdmin> = [
        ...
        },
        {
            ...
            render: (_, rec) => rec.deletable ?
                ...
                : <Tag color="magenta" key="Super Admin">Super Admin</Tag>
        }
    ]
    ...
    function getData() {
        return transformSessions(kpList).map((item) => {
            const partChild: DataNode[] = item.parts?.map((v: { partinfo: any; sessions: any[] }) => {
                const partTitle = `${v.partinfo.firstName} ${v.partinfo.lastName}(${v.partinfo.email})`
                const sessionChild: DataNode[] =
                    v.sessions.sort((a: any, b: any) => { return a.startDate - b.startDate }).map(b => {
                ...
                return { title: partTitle, key: `${item.kin.id}+${v.partinfo.id}`, children: sessionChild }
            })
    ...
    function transformSessions(sessions: any): any[] {
        ...
        for (const session of sessions) {
            ...
            for (const participant of session.participants) {
                ...
                let partOutput = kinOutput.parts.find((item: any) => item.partinfo.id === partId)
    ...
    const getUploadFileList = (pid:string) => {
        getPartFileList(pid).then((resp) => {
            ...
            if (err) {
                ...
                console.log(res)
                let fileList = res.filter((item :any | null) => item !== null).map((v :any | null) => {
                    return {
                        uid: v.id,
                        name: v.fileName,
                        status: 'done',
    ...
    const props: UploadProps = {
        ...
        onChange(info:any) {
            ...
            if (info.file.status === 'done') {
                ...
                newAttchment(currentKId,currentPId, info.file.name, uploadFilePath).then((resp) => {
    ...
    // @ts-ignore
    return <Page title="K&P Relationship" loading={loading}>
        ...
        <div>
            ...
            <Modal
                ...
                onOk={handleOk}>
                <Upload {...props}>
                    <Button icon={<UploadOutlined />}>Click to Upload Patient's signed referral form</Button>
                </Upload>
            </Modal>
        </div>
    </Page>
