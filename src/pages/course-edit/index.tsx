import { Alert, Button, Form, Input, message, Select, Space, Upload, UploadProps } from 'antd'
import { FunctionComponent, LegacyRef, useEffect, useMemo, useRef, useState } from 'react'
                ...
                {(fields, { add, remove }) => (
                    <>
                        {fields.map(({ key, name, ...restField }) => (
                            <Space key={key} direction="vertical" size="small" style={{ display: 'flex', marginBottom: 4 }} wrap={true} align="start">
                                <Form.Item
                                    {...restField}
                                    name={[name, 'kind']}
                                    label="Type"
...
async function images_upload_handler (blobInfo: any, progress: any): Promise<string> {
    ...
    if (info && info.putUrl && info.url) {
        ...
        const [, uploadErr] = await tryCatch(axios.put(info?.putUrl, blobInfo.blob(), {
            headers: {
